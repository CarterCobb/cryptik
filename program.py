...
from algorithms.rsa import RSA, RSAKeyGen
...
class Criptak:
    ...
    def __init__(self):
        ...
        get_algo = lambda: self.algorithm if self.algorithm is not None else inquirer.prompt([inquirer.List('algorithm', message='Choose an algorithm', choices=
            ['bacon', 'new-bacon', 'conceal' 'reverse', 'transposition', 'caesar', 'multi', 'affine', 'affine-hack', 'freq', 'vig', 'hack-vig', 'rsa', 'rsa-key-gen']
        )])['algorithm']
        ...
        elif self.algorithm == 'rsa-key-gen': self.algorithm = RSAKeyGen()
